...
public class CrossOverBLXAlfa implements CrossOver {
	...
	public double[][] getOffSpring(double[] vars1, double[] vars2, double[] lowerBounds, double[] upperBounds) {
		if (vars1.length != vars2.length || vars1.length != lowerBounds.length || vars1.length != upperBounds.length) {
			throw new IllegalArgumentException("Os arrays de entrada devem ter o mesmo comprimento.");
		}
